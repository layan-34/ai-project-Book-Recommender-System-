...
# Load the dataset
book_data = pd.read_csv(r'BX-Books.csv', delimiter=";", usecols=["ISBN", "Book-Title", "Book-Author"] ,encoding='latin-1')
...
data = pd.read_csv(r'BX-Book-Ratings.csv', delimiter=';', encoding='latin-1')
...
def evaluate_metrics_dynamic_thresholds(user_id,model, test_data, user_mapping, isbn_mapping, top_k=5, step=0.005):
    ...
    for isbn in top_indices:
        ...
        if isbn_mapped is not None:
            ...
            if book_info:
                ...
                recommended_books.append(book_info['title'])
                recommended_count += 1  


    return {
        "Accuracy": accuracy_score(test_interactions, (predictions > best_threshold).astype(int) ),
        "Precision": precision_score(test_interactions, (predictions > best_threshold).astype(int)),
        "recall": recall_score(test_interactions, (predictions > best_threshold).astype(int)),
        "F1_Score": f1_score(test_interactions, (predictions > best_threshold).astype(int)),
